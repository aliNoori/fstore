...
async function fetchAddress() {
  try {
    const response = await $axios.get(`user/show/address/${route.params.id}`);
    address.value = response.data.address;
    console.log('جزئیات آدرس:', address.value.street);
    form.value.street = address.value.street;
    form.value.city = address.value.city;
    form.value.state = address.value.state;
    form.value.postal_code = address.value.postal_code;
    form.value.country = address.value.country;
  } catch (error) {
    console.error('خطا در دریافت جزئیات آدرس:', error);
  }
}
const handleSubmit = async () => {
  try {
    ...
    const response = await $axios.post(`user/edit/address/${route.params.id}`, formData);
    if (response.data) {
      alert('Edit Registration successful!');
      // تغییر مسیر به صفحه دیگر بعد از ورود موفق
      await router.push('/steps-to-order');
      // Optionally redirect or clear form
    } else {
      alert('Edit Registration failed: ' + response.data.message);
    }
  } catch (error) {
    console.error('Edit Registration failed:', error);
